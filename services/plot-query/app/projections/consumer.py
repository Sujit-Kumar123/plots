...
async def _publish_to_dlq(
    ...
    try:
        ...
        logger.error(
            "Failed to publish to DLQ %s: %s — event will be lost", dlq_topic, dlq_exc
        )
...
async def run_consumer() -> None:
    ...
    logger.info("Plot query projector consumer started (manual commit mode)")
    try:
        async for msg in consumer:
            ...
            if handler:
                await _handle_with_retry(
                    handler, msg.value, dlq_producer, msg.topic, event_type
                )
            else:
        ...
        logger.info("Plot query projector consumer stopped")
