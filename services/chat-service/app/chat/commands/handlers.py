...
from app.chat.schemas.request.message_commands import (
    BulkCreateChatMessagesCommand,
    CreateChatMessageCommand,
    DeleteChatMessageCommand,
)
from app.config import settings
from app.chat.events import publisher
from app.chat.events.schemas import (
    ChatBulkMessagesCreatedEvent,
    ChatMessageCreatedEvent,
    ChatMessageDeletedEvent,
)
from app.chat.models.write_models import ChatMessage, ChatSession


async def handle_create_message(
    cmd: CreateChatMessageCommand, user_id: uuid.UUID, db: AsyncSession
) -> ChatMessage:
    # Frontend generates session UUID from localStorage — auto-create on first message
    result = await db.execute(
        select(ChatSession).where(ChatSession.id == cmd.session_id)
    )
