...
from app.email.schemas.request.email_requests import SendBulkEmailRequest, SendEmailRequest
from app.email.service.email_service import send_email
...
@router.post("/send", status_code=status.HTTP_200_OK, summary="Send a single transactional email",
    description=(
        "Renders a Jinja2 template from `email/templates/` and sends it via SendGrid. "
        "Requires the `x-internal-secret` header. If `SENDGRID_API_KEY` is unset the email is silently skipped."
    ),
    responses={**_ERR},
)
async def send_single(body: SendEmailRequest):
    ...
    return ResponseHandler.ok("Email sent")

@router.post("/send-bulk", status_code=status.HTTP_200_OK, summary="Send multiple transactional emails",
    responses={**_ERR},
)
async def send_bulk(body: SendBulkEmailRequest):
    ...
    return ResponseHandler.ok(data={"sent": sent, "failed": len(errors), "errors": errors})
